...
#!/data/data/com.termux/files/usr/bin/bash
# Generated by setup.sh — absolute paths baked in, just run: bash start.sh
pm2 delete termuxhost-api 2>/dev/null || true
pm2 start $DIST --name termuxhost-api --node-args="--env-file=$ENV_FILE"
pm2 save
sleep 2
curl -s http://localhost:\$(grep '^PORT=' $ENV_FILE | cut -d= -f2)/api/healthz && echo "" || echo "[!] Server not responding — run: pm2 logs termuxhost-api"
STARTEOF
